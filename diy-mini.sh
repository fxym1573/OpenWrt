...
sed -i '$i uci set nlbwmon.@nlbwmon[0].refresh_interval=2s' package/default-settings/files/zzz-default-settings
sed -i '$i uci commit nlbwmon' package/default-settings/files/zzz-default-settings
...
# 修改本地时间格式
sed -i 's/os.date()/os.date("%a %Y-%m-%d %H:%M:%S")/g' package/autocore/files/*/index.htm
...
# 取消对 samba4 的菜单调整
# sed -i '/samba4/s/^/#/' package/default-settings/files/zzz-default-settings
